        ...
        <div className={`mobius-chat-input-editor min-w-0 flex-shrink-0 ${compact ? 'p-1' : 'p-2'}`}>
          ...
          <div className={`flex items-center justify-between ${compact ? 'pt-0.5' : 'pt-1'}`}>
            ...
            <div className="flex items-center gap-1">
              ...
              <button type="button" aria-label={state.sending ? '发送中' : '发送指令'} title={state.sending ? '发送中…' : '发送指令'} disabled={state.sending || !state.draft.trim()} onClick={() => { void send() }} className={`agent-conversation-overlay__send rounded-md transition-colors disabled:opacity-40 ${compact ? 'p-0.5' : 'p-1.5'}`} style={{ color: session.color, background: `${session.color}18` }}>{state.sending ? <RefreshCw className={`${compact ? 'h-3 w-3' : 'h-3.5 w-3.5'} animate-spin`} /> : <Send className={compact ? 'h-3 w-3' : 'h-3.5 w-3.5'} />}</button>
            </div>
          </div>
        </div>
      </div>
